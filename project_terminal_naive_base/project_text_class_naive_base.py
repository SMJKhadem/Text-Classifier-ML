...
import os
import pandas as pd
os.chdir("/Users/kasiafalecka/Desktop/spiced_projects/nigela-network-student-code/week_04/project/project_terminal_naive_base/")
path=os.getcwd()
path
...
for i, names in enumerate(song_tit_nicky):
          ...
          if song_tit_nicky[i] is None:
               song_tit_nicky[i]="a"
            
for i, names in enumerate(song_tit_Pink):
          song_tit_Pink[i]= song_tit_Pink[i].replace("/", "")
          if not song_tit_Pink[i]:
               print("injahaa")
               song_tit_Pink[i]="abc"

#np.where(song_tit_Pink=="a")
          
#"abc" in song_tit_Pink
...
m = MultinomialNB(force_alpha=True)
...
sing_predict=Pipeline([
     ...
     ("model", GridSearchCV(estimator=m, 
                    param_grid={"alpha": np.logspace(-4,1)}, 
                    cv=5, 
                    scoring='',
                    verbose = 3))
...
# In[453]:
sing_predict.score(X_test, y_test)
#sing_predict.predict(['we need no education'])

# In[454]:
import pickle
with open('guess_singer_naive.pkl', 'wb') as f:
